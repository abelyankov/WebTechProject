...
export class ProviderService extends MainService  {
  ...
  getMyTasks(id: string): Promise<ITasks[]> {
  ...
  postExecuteTask(task_id: number, user_id: string): Promise<ITaskExecute> {
    return this.put(`http://127.0.0.1:8000/executeTask/?userId=` + user_id + '&taskId=' + task_id , {});
  }

  updateTask(task: ITaskDetail) {
    return this.put(`http://127.0.0.1:8000/taskList/${task.id}/`, {
      title: task.title, description: task.description, address: task.address, duration: 1, created_by: 1, status: 2
    });
  }
